...
func ghRule(req ScreeningRequest, indexedList IndexedWatchlist) MatchResult {
	...
	for _, entry := range indexedList.Entries {
		...
		if hasLastName {
			lastNameHit, lastNameScore = evaluateFuzzyMatch(entry.LastName, req.LastName, 90.0)
		}
