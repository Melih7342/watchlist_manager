...
func mvRule(req ScreeningRequest, indexedList IndexedWatchlist) MatchResult {
	...
	for _, entry := range indexedList.Entries {
		...
		if hasFirstName {
			firstNameHit, firstNameScore = evaluateFuzzyMatch(entry.FirstName, req.FirstName, 75.0)
		}
		if hasLastName {
			lastNameHit, lastNameScore = evaluateFuzzyMatch(entry.LastName, req.LastName, 75.0)
		}
